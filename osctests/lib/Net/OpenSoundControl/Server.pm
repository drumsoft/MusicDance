...
=item new(Port => $port, Name => $name, Handler => \&handler, MaxLength => $max_length)
...
MaxLength is the maximum size of OSC packets which the clients will send. Its default is 1024.
...
sub new {
    ...
    $self->{MAXLEN} = $opts{MaxLength} || 1024;
...
To exit the readloop, call stoploop() in your Handler subroutine or some signal handlers (will break RECV blocking).
